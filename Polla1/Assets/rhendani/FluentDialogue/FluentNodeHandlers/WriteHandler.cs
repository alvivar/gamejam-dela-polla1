...
namespace Fluent
{
    ...
    public class WriteHandler : FluentNodeHandler
    {
        ...
        private IEnumerator TypeText()
        {
            ...
            if (allText.Length > 0)
            {
                ...
                while (true)
                {
                    ...
                    currentPosition++;
                    // FASTER IF
                    if (Input.GetKeyDown(KeyCode.E)) faster = true;
                    if (faster) currentPosition += 4;
                    //print("THIS IS HERE");
            ...
            {
                print("RUNNING WRITING CHAT");
                // if (ChatReady) ChatReady = false;
                textTextUI.text = "";
            }
            ...
            // if (currentNode.WaitForNext) currentNode.Done();
        ...
        void Update()
        {
            // if (Input.GetKeyDown(KeyCode.E)) StartCoroutine("TypeText");
            bool has = false;
            if (currentNode != null) has = currentNode.WaitForButtonPress;
            if (has && wrote && Input.GetKeyDown(KeyCode.E))
            {
