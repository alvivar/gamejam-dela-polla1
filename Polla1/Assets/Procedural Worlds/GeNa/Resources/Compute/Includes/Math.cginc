...
float sdf_shape(float3 p, float radius, SdfShape s)
{
    ...
    if (s.data0.x == 0) return sdf_sphere(p, radius, s.data1.xyz, s.data1.w);
    if (s.data0.x == 1) return sdf_box(p, radius, s.data1.xyz, s.data2.xyz, s.data3, s.data1.w);
    if (s.data0.x == 2) return sdf_capsule(p, radius, s.data1.xyz, s.data2.xyz, s.data1.w);
    if (s.data0.x == 3) return sdf_cylinder(p, radius, s.data1.xyz, s.data2.xyz, s.data1.w);
    // Return Infinity
    return 1e32f;
}
